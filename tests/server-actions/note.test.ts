...
import { mockFs, mockRevalidatePath, resetAllMocks, createFormData } from '../setup'
...
vi.mock('@/app/_utils/yaml-metadata-utils', () => ({
  ...
  generateYamlFrontmatter: vi.fn().mockReturnValue('---\nuuid: test-uuid-123\n---\n'),
  extractYamlMetadata: vi.fn().mockReturnValue({ metadata: { uuid: 'test-uuid-123' }, contentWithoutMetadata: 'Test content' }),
  updateYamlMetadata: vi.fn().mockReturnValue('---\nuuid: test-uuid-123\n---\nTest content'),
...
describe('Note Actions', () => {
  beforeEach(() => {
    ...
    mockGetCurrentUser.mockResolvedValue({ username: 'testuser', fileRenameMode: 'minimal' })
  ...
  describe('createNote', () => {
    ...
    it('should use user from formData when provided', async () => {
      const formData = createFormData({
        ...
        category: 'TestCategory',
        rawContent: 'Content',
        user: JSON.stringify({ username: 'apiuser', fileRenameMode: 'minimal' }),
      })
