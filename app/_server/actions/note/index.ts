      ...
      if (parts.length >= 3) {
        ...
        try {
          ...
          if (notes.success && notes.data) {
            ...
            if (note?.uuid) {
              const newLink = `[${linkText}](/jotty/${note.uuid})`;
              convertedContent = convertedContent.replace(fullMatch, newLink);
            } else {
            }
          }
        } catch (error) {
          console.warn("Failed to convert note link:", linkHref, error);
        }
      }
    }
    else if (linkHref.startsWith("/checklist/")) {
      const parts = linkHref.split("/");
      if (parts.length >= 3) {
        const categoryAndId = parts.slice(2).join("/");
        const lastSlashIndex = categoryAndId.lastIndexOf("/");
        ...
        try {
          ...
          if (checklists.success && checklists.data) {
            ...
            if (checklist?.uuid) {
              const newLink = `[${linkText}](/jotty/${checklist.uuid})`;
              convertedContent = convertedContent.replace(fullMatch, newLink);
            } else {
              console.log("Checklist has no UUID, skipping conversion");
            }
          }
        } catch (error) {
          console.warn("Failed to convert checklist link:", linkHref, error);
        }
      }
    }
    else if (linkHref.startsWith("jotty://")) {
      const uuid = linkHref.replace("jotty://", "");
      const newLink = `[${linkText}](/jotty/${uuid})`;
      convertedContent = convertedContent.replace(fullMatch, newLink);
    }
  }
