...
>(({ items, command }, ref) => {
    ...
    const renderSection = (title: string, items: AtMentionItem[], icon: React.ReactNode, startIndex: number) => (
    ...
    return (
        <div className="bg-card border border-border rounded-lg shadow-lg p-3 min-w-80 max-w-96">
            ...
            <div className="max-h-80 overflow-y-auto">
                {allItems.length ? (
                    <>
                        {notes.length > 0 && renderSection("Notes", notes, <BookText className="h-3 w-3" />, 0)}
                        {checklists.length > 0 && renderSection("Checklists", checklists, <CheckSquare className="h-3 w-3" />, notes.length)}
                    </>
                ) : (
