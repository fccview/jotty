...
export const useStyling = () => {
  ...
  const handleEditTheme = async (themeId: string) => {
    ...
    if (
      !rawConfig ||
      !rawConfig["custom-themes"] ||
      !rawConfig["custom-themes"][themeId]
    ) {
      showToast({
        type: "error",
    ...
    setThemeForm({
      ...
      colors: {
        ...
        "--secondary-foreground":
          processedColors["--secondary-foreground"] || "",
        ...
        "--destructive-foreground":
          processedColors["--destructive-foreground"] || "",
  ...
  const handleSaveTheme = async () => {
    ...
    try {
      ...
      Object.keys(processedColors).forEach((key) => {
        ...
        if (value && value.startsWith("#")) {
          processedColors[key as keyof typeof processedColors] =
            hexToRgb(value);
        }
      });

      const themeId =
        editingTheme || themeForm.name.toLowerCase().replace(/\s+/g, "-");
